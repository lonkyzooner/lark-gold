...
export class OpenAIVoiceService {
  ...
  public async speak(text: string, voice = DEFAULT_VOICE, streamingEnabled = ENABLE_STREAMING): Promise<void> {
    ...
    // Preprocess command text with Rust/WASM
    try {
      const { processCommand } = await import('../../lib/voice-wasm');
      text = await processCommand(text);
    } catch (error) {
      console.warn('[OpenAIVoice] WASM command processing failed, using original text:', error);
    }
